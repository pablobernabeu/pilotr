...
R package additionally offers precision/ROPE design analysis, the `lme4`
reference backend for mixed-effects power, the `brms` bridge and the app
launcher.
...
which solves that curve for the sample size a target power needs and
reports an interval on it.
