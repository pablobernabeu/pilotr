...
`n_converged`, `n_singular`, and `n_warning`. Each decision proportion
is reported with its Monte Carlo standard error (`*_mcse`) and Wilson
interval bounds (`*_lo`, `*_hi`), because a proportion over a finite
number of replicates is an estimate rather than a fact. The interval
