...
Embed the specification as an R list literal, so that the returned
script reproduces the design without any external file. This turns a
design built in the no-code application into a reproducible script; the
application's Verify button runs that script in a clean R session and
confirms that it reproduces the data bit-for-bit.
...
Numbers are emitted at 17 significant digits rather than through
[`deparse()`](https://rdrr.io/r/base/deparse.html), which prints 15 and
so does not round-trip: `deparse(1/3)` reads back as a different double.
Since the point of the script is bit-for-bit reproduction, the embedded
specification has to preserve every coefficient exactly.
