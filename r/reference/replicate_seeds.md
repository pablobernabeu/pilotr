...
computed once rather than any work per replicate. Duplicates are
skipped, so no two replicates are handed the same seed and silently
produce identical data, and the skipping is deterministic, so the R and
'Python' implementations still agree.
