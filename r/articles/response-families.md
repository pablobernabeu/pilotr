...
crossing, is available by writing the specification directly, as shown
next.
...
intercept means. A shifted lognormal is not a substitute, because
...
does not cover it, so an ex-Gaussian design is written directly, in the
way the next section shows.
