...
* **`spec_version`** negotiates the format. A specification with no such field is a 0.2
  ...
  differently and generates different data without complaint; and one declaring a version newer than
...
* **`varies_by = "observation"`** draws a predictor once per row, for a quantity that varies trial by
  trial. `varies_by` is also validated now: **anything other than `"subject"` was previously read as
  item-level**, so a predictor declared to vary by `"trial"` was silently given one value per item.
