...
ex-Gaussian, continuous predictors, interactions, nesting and partial crossing, is available by
...
is not a substitute, because `model_data()` logs the response back and leaves a symmetric
residual on the analysis scale. `build_spec()` does not cover it, so an ex-Gaussian design is
written directly, in the way the next section shows.
