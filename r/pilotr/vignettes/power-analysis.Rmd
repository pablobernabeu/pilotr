...
by-subject and by-item designs. The R interface derives the analysis model from the
specification, fits it with `lme4`/`lmerTest` and tests each fixed effect with Satterthwaite
degrees of freedom.
...
The model fitted is the one the specification implies, contrast columns, response transform and
all, so nothing about the analysis has to be written out by hand.
...
pm <- readRDS("power-mixed-cache.rds")
```
...
The fit accounting is reported in full because a mixed model can return a usable estimate while
still sitting on a boundary. `n_returned` counts the replicates that yielded a fit, and it is the
denominator of `power`. The remaining counts break those fits down: `n_converged` are the ones
with neither a warning nor a singular fit, and here they are a minority, which says that a
maximal random-effect structure over 24 subjects and 18 items is richer than this design can
support. That is common in small crossed designs, and worth reporting beside the power rather
than hidden behind it. Singular and warning fits are kept in the rate, because their
fixed-effect estimates remain interpretable and dropping them would bias the result:
singularity is not independent of the variance estimates that produce it.
...
curve <- power_curve_mixed(
  ...
  n_sims = 50)
curve[, c("n_subject", "power", "power_mcse", "type_m", "n_returned",
          "n_converged", "n_singular")]
```
...
The sweep also returns the Wilson interval on each power estimate, `power_lo` and `power_hi`,
along with the Type S error and the rest of the fit accounting. `n_singular` drifts down as the
sample size rises, which is the design starting to support the model it is analysed under.
...
library(ggplot2)
# Each power estimate is a proportion over a finite number of replicates, and
# the sweep reports its 95% Wilson interval, which is what the band draws.
...
The shaded horizontal band along the curve is the Monte Carlo interval, the Wilson interval the
sweep reports for each power estimate. The vertical band is the solved sample size and its own
interval, which is what the dashed target line invites the reader to guess at.
