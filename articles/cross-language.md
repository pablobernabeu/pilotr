...
The repository includes a parity check that simulates both designs in
both languages and reports the maximum absolute difference. That
difference is 0.0 across thousands of rows, with no categorical
mismatches. The simulation core is also bit-identical across machines
and R versions, so a design verified on a laptop reproduces exactly on
an HPC node.
