...
- n_sims:
  ...
  `p_equivalent` are proportions over the converged replicates, so they
  carry a Monte Carlo standard error of about
...
- workers:
  ...
  count returns results identical to a serial run. The worker pool is
  created once and reused across the sweep.
