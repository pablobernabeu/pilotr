...
Add the analysis response column `.y` (log-transformed for the lognormal
families), the numeric contrast columns implied by the categorical
factors, and any interaction product columns (an `a:b` coefficient
becomes a column `a_b`).
...
- d:
  ...
  A simulated data set, as returned by
